...
library(tidyverse)
#library(here)# shouldn't need this for this lab
library(rio)
``` 


```{r load data, include = FALSE}
# No need to namespace since you loaded rio above
alcohol <- import("https://github.com/fivethirtyeight/data/raw/master/alcohol-consumption/drinks.csv", 
                  setclass = "tbl_df")
...
**DA:** I like it. I'd actually have gone even slightly lower, like maybe 15.
...
**DA:** So you're actually looking at the wrong thing here. You're supposed to viewing total alcohol liters, but you're looking at wine servings. That said, I think the binwidth is too high. The data are too smoothed and become obscured. 
...
**DA:** This is a fine plot but does not get at the distribution of total alcohol consumption.
...
**DA** I think these plots have too large and too small of binwidths. I woul go for something in-between, like 0.5.
...
?str_to_title # Remove these sorts of things before submitting
...
e %>% 
  ...
  geom_boxplot(fill = "#56B4E9", alpha = 0.9, width = 0.5)+
  theme_minimal(base_size = 15)+
  labs(x = "Alcohol Types", 
       y = "Average Consumption", 
       title = "Average Consumption of Wine, Spirit, and Beer in Countries \n with More Than 10 Liters Pure Alcohol Consumption")

# DA: Careful on your code styling
```
...
**DA**: Great that you're exploring, but be sure to answer the questions of the lab, not just those that you are interested in.
...
f %>% 
  ...
  labs(x = "Top Countries", 
       y = "Proportion of Alcohol Consumed", 
       title = "Alcohol Consumption by Type", 
       subtitle = "Top 25 countries displayed (according to total liters consumed)")

# This is great! We'll call it close enough and you can have the extra point.
```
