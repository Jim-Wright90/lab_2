...
Claire: I noticed that the question is about total alcohol consumded so I thought maybe just focus on this column. And firstly I want to investigate the total alcohol consumption by countries but I can only use geom_col instead of geom_histogram since I have two variables. Still, it was a fun exploration, see graph "Top 20 Countries in Alcohol Consumption" below.
...
Then I tried to change some selections and played with different binwidths but with the bw getting bigger, more details get lost due to the smoothing, even I only set bw = 5, I'm not seeing it telling us correct info about the counts of total alcohol: 
```{r}
ggplot(alcohol, aes(total_litres_of_pure_alcohol)) +
...
So I finally set bw at 0.1 since it tells me more details and allows me to see the highly different alcohol consumptions all over the world.
```{r}
ggplot(alcohol, aes(total_litres_of_pure_alcohol)) +
...
Claire: my second question is in the countries with lager alcohol consumption (pure alcohol liters >= 10), what are people's favorite, wine, spirit, or beer? The plot below indicates that beer is still the most welcomed, but wine is consumed more than spirit in average. The mean differences are still not statistically significant though.
```{r}
e <- d %>% 
  arrange(desc(total)) %>% 
  filter(total >= 10)
...
2. Reproduce plot
Claire: this is really really hard.
