...
ggplot(c, aes(total)) +
  geom_density()
```
Then I tried to change some selections and played with different binwidths but with the bw getting bigger, more details get lost due to the smoothing, even I set bw = 1, I'm not seeing it telling us correct info about the counts of total alcohol: 
```{r}
ggplot(c, aes(total)) +
  geom_density(fill = "#56B4E9", bw = 1)
```
So I set bw = 0.1.
```{r}
ggplot(c, aes(total)) +
  geom_density(fill = "#56B4E9", bw = 0.1)
```
